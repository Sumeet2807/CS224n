...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        decoder_out = self.charDecoder(input_embedded,dec_hidden)
        char2id_out = self.char_output_projection(decoder_out[0])
        return(char2id_out,decoder_out[1])
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        x_entropy_loss = torch.nn.CrossEntropyLoss()
        in_seq = char_sequence[:-1,:]
        target_seq = torch.flatten(char_sequence[1:,:])
        scores, dec_out = self.forward(in_seq, dec_hidden)
        scores = torch.reshape(scores,(scores.shape[0]*scores.shape[1],scores.shape[2]))
        scores[:,0] = 0
        loss = x_entropy_loss(scores,target_seq)
        return(loss)
