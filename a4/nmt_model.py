...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # self.decoder = torch.nn.LSTM(embed_size+hidden_size,hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = torch.nn.utils.rnn.pack_padded_sequence(self.model_embeddings.source(source_padded),source_lengths)
        out, hc_n = self.encoder(X)
        h_n, c_n = hc_n
        enc_hiddens = torch.transpose(torch.nn.utils.rnn.pad_packed_sequence(out)[0],0,1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # out, dec_state = self.decoder(torch.unsqueeze(Ybar_t,0),(torch.unsqueeze(dec_state[0],0),torch.unsqueeze(dec_state[1],0)))
        # h_n, c_n = dec_state
        # e_t = torch.squeeze(torch.bmm(enc_hiddens_proj,torch.unsqueeze(torch.squeeze(h_n,0),2)),2)
        #**********************************************************************************
        dec_state = self.decoder(Ybar_t, dec_state)
        h_n, c_n = dec_state
        e_t = torch.squeeze(
            torch.bmm(enc_hiddens_proj, torch.unsqueeze(h_n, 2)), 2)
        ...
        # U_t = torch.cat((a_t,torch.squeeze(h_n,0)),1)
        U_t = torch.cat((a_t, h_n), 1)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # hyp_word_ids = torch.floor_divide(top_cand_hyp_pos,len(self.vocab.tgt))
