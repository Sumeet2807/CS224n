...
def minibatch_parse(sentences, model, batch_size):
    ...
    for sentence in sentences:
        ...
        parse_to_index[parse] = i
        i+=1
    unfinished_parses = copy.copy(partial_parses)
    while len(unfinished_parses):  	
        parse_steps = model.predict(unfinished_parses[:batch_size])
        for parse in unfinished_parses[:batch_size]:
            parse.parse_step(parse_steps.pop(0))
            if (len(parse.buffer) == 0) and (len(parse.stack) <=1) :
                unfinished_parses.remove(parse)
                dependencies[parse_to_index[parse]] = parse.dependencies

    		

    # index = 0
    # while len(partial_parses)-1 > index + batch_size:
    #     parse_steps = model.predict(partial_parses[index:(index + batch_size)])
    #     i=0
    #     for step in parse_steps:
    #         print(step)
    #         partial_parses[i].parse_step(step)
    #          print(partial_parses[i].dependencies)
    #         dependencies.append(partial_parses[i].dependencies)
    #         i += 1
    #     index = index + batch_size
    # parse_steps = model.predict(partial_parses[index:len(partial_parses)])
    # i=0
    # for step in parse_steps:
    #     partial_parses[i].parse_step(step)
    #     dependencies.append(partial_parses[i].dependencies)
