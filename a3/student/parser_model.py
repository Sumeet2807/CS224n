...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.nn.init.xavier_uniform_(torch.empty((n_features*embeddings.shape[1],hidden_size))))
        self.embed_to_hidden_bias = nn.Parameter(torch.nn.init.uniform_(torch.empty((1,hidden_size))))
        self.hidden_to_logits_weight = nn.Parameter(torch.nn.init.xavier_uniform_(torch.empty((hidden_size,n_classes))))
        self.hidden_to_logits_bias = nn.Parameter(torch.nn.init.uniform_(torch.empty((1,n_classes))))
    ...
    def forward(self, w):
        ...
        h = self.dropout(torch.nn.functional.relu(torch.matmul(self.embedding_lookup(w),self.embed_to_hidden_weight) + self.embed_to_hidden_bias))
